...
class MetaChain:
    ...
    def run_and_stream(
        ...
        # Initialize consecutive tool call tracking to prevent infinite loops
        last_tool_name = None
        consecutive_tool_calls = 0
        MAX_CONSECUTIVE_TOOL_CALLS = 10  # Maximum allowed consecutive calls of the same tool
        ...
        while len(history) - init_len < max_turns:
            ...
            # Track usage statistics from streaming response (if available in last chunk)
            if last_chunk and hasattr(last_chunk, 'usage') and last_chunk.usage:
                usage_stats["api_calls"] += 1
                usage_stats["total_prompt_tokens"] += getattr(last_chunk.usage, 'prompt_tokens', 0)
                usage_stats["total_completion_tokens"] += getattr(last_chunk.usage, 'completion_tokens', 0)
                usage_stats["total_tokens"] += getattr(last_chunk.usage, 'total_tokens', 0)
                # Calculate cost using litellm's completion_cost function
                try:
                    cost = completion_cost(completion_response=last_chunk)
                    usage_stats["total_cost"] += cost
                except Exception:
                    pass  # Cost calculation may fail for streaming responses
            else:
                # Count API call even if usage info not available
                usage_stats["api_calls"] += 1
            ...
            # Track consecutive tool calls to prevent infinite loops
            current_tool_name = tool_calls[0].function.name if tool_calls else None
            ...
            # Check if consecutive tool calls exceed the limit
            if consecutive_tool_calls > MAX_CONSECUTIVE_TOOL_CALLS:
                debug_print(
                    debug,
                    f"Tool '{current_tool_name}' has been called {consecutive_tool_calls} times consecutively. "
                    f"Breaking to prevent infinite loop."
                )
                # Add tool response messages for each tool_call to satisfy API requirements
                for tool_call in tool_calls:
                    tool_response = {
                        "role": "tool",
                        "tool_call_id": tool_call.id,
                        "name": tool_call.function.name,
                        "content": f"[SYSTEM] Tool call aborted: You have called '{tool_call.function.name}' {consecutive_tool_calls} times consecutively. "
                                   f"This may indicate an infinite loop. Please try a different approach or tool."
                    }
                    history.append(tool_response)
                # Reset counter after warning
                consecutive_tool_calls = 0
                last_tool_name = None
                continue  # Let the agent reconsider with the tool response
    ...
    def run(
        ...
        while len(history) - init_len < max_turns and active_agent:
            ...
            if hasattr(completion, 'usage') and completion.usage:
                usage_stats["api_calls"] += 1
                usage_stats["total_prompt_tokens"] += getattr(completion.usage, 'prompt_tokens', 0)
                usage_stats["total_completion_tokens"] += getattr(completion.usage, 'completion_tokens', 0)
                usage_stats["total_tokens"] += getattr(completion.usage, 'total_tokens', 0)
                ...
                try:
                    ...
                    if debug:
                        self.logger.info(f"Could not calculate cost: {str(e)}", title="Cost Tracking", color="yellow")
            ...
            if message.tool_calls:
                # Track consecutive tool calls to prevent infinite loops
                current_tool_name = message.tool_calls[0].function.name if message.tool_calls else None
                if current_tool_name == last_tool_name:
                    consecutive_tool_calls += 1
                else:
                    consecutive_tool_calls = 1
                ...
                if consecutive_tool_calls > MAX_CONSECUTIVE_TOOL_CALLS:
                    self.logger.info(
                        f"Tool '{current_tool_name}' has been called {consecutive_tool_calls} times consecutively. "
                        f"Breaking to prevent infinite loop.",
                        title="Consecutive Tool Call Limit",
                        color="yellow"
                    )
                    # Add tool response messages for each tool_call to satisfy API requirements
                    for tool_call in message.tool_calls:
                ...
                # Reset consecutive tool call tracking when no tool is called
                consecutive_tool_calls = 0
                last_tool_name = None
            history.extend(partial_response.messages)
            context_variables.update(partial_response.context_variables)
