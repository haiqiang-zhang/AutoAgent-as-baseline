...
def _accumulate_usage_stats(total_stats: dict, new_stats: dict):
    """Helper function to accumulate usage statistics."""
    if new_stats:
        total_stats["total_prompt_tokens"] += new_stats.get("total_prompt_tokens", 0)
        total_stats["total_completion_tokens"] += new_stats.get("total_completion_tokens", 0)
        total_stats["total_tokens"] += new_stats.get("total_tokens", 0)
        total_stats["total_cost"] += new_stats.get("total_cost", 0.0)
        total_stats["api_calls"] += new_stats.get("api_calls", 0)
...
async def run_in_client(
    ...
    # Initialize accumulated usage statistics
    total_usage_stats = {
    ...
    for i in range(MAX_RETRY):
        try:
            ...
            # Accumulate usage stats from this call
            _accumulate_usage_stats(total_usage_stats, response.usage_stats)
...
"""
                ...
                # Accumulate usage stats from meta_agent call
                _accumulate_usage_stats(total_usage_stats, response.usage_stats)
                if 'Case resolved' in response.messages[-1]['content']:
                    break
                else:
    ...
    # Set accumulated usage stats on the final response
    response.usage_stats = total_usage_stats
    return response
