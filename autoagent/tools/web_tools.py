...
def _serpapi_search(query: str, num_results: int = 20, location: str = None) -> str:
    """
    ...
    Args:
        ...
        location: Optional location for localized results (e.g., "Austin, Texas, United States").
    ...
    if not api_key:
        raise ValueError(
            "SerpAPI credentials not found. "
            "Please set SERPAPI_API_KEY environment variable."
        )
    ...
    try:
        ...
        results.append(f"Total results: {total_results}\n")
        ...
        if "answer_box" in data:
            ...
            if "answer" in answer_box:
                results.append(f"\n## Answer Box")
                results.append(f"{answer_box['answer']}\n")
            elif "snippet" in answer_box:
                results.append(f"\n## Answer Box")
                results.append(f"{answer_box['snippet']}\n")
        ...
        for idx, item in enumerate(data["organic_results"][:num_results], 1):
            ...
            results.append("it is texted and url based results, please do not use any browser tool to visit the link.(click, page_down, etcs.) But it contain urls, you can use `visit_url` tool to visit the link if necessary.")

            # Add date if available
            if "date" in item:
                results.append(f"**Date:** {item['date']}")
...
def web_search(context_variables, query: str, location: str = None):
    """
    Performs a web search using SerpAPI (default) or browser-based search (fallback).
    ...
    Args:
        ...
        location: Optional location for localized results (e.g., "Austin, Texas, United States").
    """
    # Try API-based search first if enabled
    # if use_api:
    
    try:
        search_results = _serpapi_search(query, location=location)
        ...
        # API credentials not configured or serpapi not installed, fall back to browser search
        print(f"Warning: {e}. Falling back to browser-based search.")
    except Exception as e:
        print(f"Error with API search: {e}. Falling back to browser-based search.")

    # # Fallback to browser-based search
    # env: BrowserEnv = context_variables.get("web_env", None)
    # if env is None:
    #     return Result(
    #         value="Error: Neither SerpAPI credentials nor web_env are configured. "
    #               "Please either:\n"
    #               "1. Set SERPAPI_API_KEY environment variable and install serpapi (pip install google-search-results), or\n"
    #               "2. Ensure web_env is set in context_variables for browser-based search.",
    #         image=None,
    #     )

    # try:
    #     # Use Google search as fallback
    #     # action_str = f"_visit_page('https://www.google.com/search?q={quote_plus(query)}&hl=en')"
