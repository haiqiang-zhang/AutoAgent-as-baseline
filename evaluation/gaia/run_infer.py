...
def process_instance(
    ...
    try:
        ...
        usage_info = response.usage_stats if hasattr(response, 'usage_stats') else {}

        # Log usage statistics
        if usage_info:
            logger.info(
                f'API Calls: {usage_info.get("api_calls", 0)} | '
                f'Total Tokens: {usage_info.get("total_tokens", 0)} | '
                f'Cost: ${usage_info.get("total_cost", 0):.4f}',
                title='Usage Stats', color='cyan'
            )
